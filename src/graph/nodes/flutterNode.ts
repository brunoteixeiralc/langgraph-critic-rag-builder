...
export function createIosNode(llmClient: OpenRouterService) {
  return async (state: GraphState, runtime?: Runtime): Promise<Partial<GraphState>> => {
    ...
    const ragPromise = ragService.retrieveContext(state.initialCommand, "ios");
    ...
    const correctiveRagPromise = needsCorrectiveRag
      ? ragService.retrieveContext(state.reviewerSearchQuery!, "ios")
...
6. If a previous snippet below is EMPTY or is literally just the placeholder token itself (e.g. the string "[CODE_SNIPPET_1]"), that means you FAILED to write real code for it last attempt. This is NOT approved content to preserve — you MUST discard it and write brand-new, complete, compilable Swift code for it. Never copy an empty or placeholder-only snippet forward into your new 'codeSnippets' output.
