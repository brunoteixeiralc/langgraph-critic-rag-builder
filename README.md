...
graph TD
    ...
    IosAgent --> Reviewer[3. Reviewer Fact-Checker]
...
* **Goal**: Analyze the user prompt and classify it into a technical domain (`ios`, `node_react`, or `ai_engineering`).
...
* **Smart Folder Naming**: The Orchestrator generates a succinct folder slug (max 20 characters, e.g., `ios-swiftui`) based on the topic, optimizing directory structure and preventing operating system filesystem path length issues (`ENAMETOOLONG`).
...
* **iOS Specialist**: Focuses on Swift/SwiftUI/UIKit architecture, Apple's under-the-hood platform behavior, and clean, compilable Swift code.
...
* **Factual Accuracy**: Refuses fabricated framework versions (e.g., claiming a feature was introduced in `iOS 16` instead of `17`).
...
1. If the Reviewer rejects a post, it outputs a precise search query (e.g., `ios 17 swiftui observable macro changelog`) alongside its descriptive critique.
...
├── snippet_1.swift      # Original compilable Swift source code
├── snippet_1.png        # Rendered code image (Dracula theme, Mac window frame)
├── snippet_2.swift      # Secondary compilable snippet
...
```bash
npm start "Explain how dependency injection works in iOS using Swift property wrappers"
```
